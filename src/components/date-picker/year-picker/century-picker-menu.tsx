...
import { YEAR_DECADE } from './year-picker-menu'
...
export const YEAR_CENTURY = 100

const CenturyPickerMenu: FC<CenturyPickerMenuProps> = ({
  year,
  onChange,
  container,
}) => {
  ...
  const [startYear, setStartYear] = useState(
    +Big(Math.floor(+Big(year).div(YEAR_CENTURY))).times(YEAR_CENTURY),
  )
  ...
  const handleSelect = useCallback(({ year }: { year: number }): void => {
    onChange(year)
  }, [])

  return (
    <div
      className='century-picker-menu'
      ref={centuryPickerRef}
      onClick={handleClick}>
      <Flex
        justifyContent='spaceBetween'
        alignItems='center'
        className='century-picker-menu-header'>
        <DoubleLeftOutlined
          className='century-picker-menu-header-icon'
