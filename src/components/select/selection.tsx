import React, { Component, CSSProperties, MouseEvent, ReactNode, ReactNodeArray, Ref } from 'react'
import classNames from 'classnames'
import { isNil } from 'laamginghong-utils'
import { CloseOutlined, CloseCircleFilled, DownOutlined } from '../icon'
...
class Selection<T> extends Component<SelectionProps<T>> {
    ...
    private _handleClose = (e: MouseEvent<HTMLElement>, value: OptionProps<T>): void => {
        e.stopPropagation()
        const { selected, onClear } = this.props
        onClear((selected as OptionProps<T>[]).filter((item) => item.value !== value.value).map((item) => item.value))
    }

    private _renderSelectionMultiple = (): ReactNodeArray => {
        const { maxCount, selected, maxCountText, renderSelection } = this.props
        if (maxCount) {
            const result = (selected as OptionProps<T>[]).slice(0, maxCount).map((item) => (
                <Flex alignItems='center' className='select-selection-content-multiple-item' key={item.value + item.name}>
            ...
            if ((selected as OptionProps<T>[]).length > maxCount) {
                result.push(
                    <Flex alignItems='center' className='select-selection-content-multiple-item' key='more'>
                        {maxCountText ?? `and ${(selected as OptionProps<T>[]).length - maxCount} more`}
                    </Flex>,
                )
            }
            return result
        }
        return (selected as OptionProps<T>[]).map((item) => (
            <Flex alignItems='center' className='select-selection-content-multiple-item' key={item.value + item.name}>
                {renderSelection ? renderSelection(item) : item.name}
                <CloseOutlined
                    className='select-selection-content-multiple-item-icon'
    ...
    render():
        ...
        | null
        | undefined {
        const { style, selected, disabled, open, selectionRef, placeholder, allowClear, multiple, renderSelection } = this.props
        return (
            <div
                ref={selectionRef}
                ...
                {multiple ? (
                    <Flex
                        ...
                        alignItems='center'
                        style={{
                            lineHeight: (selected as OptionProps<T>[]).length ? 'normal' : '35px',
                        }}
                        className={classNames('select-selection-content', {
                            placeholder: !(selected as OptionProps<T>[]).length,
                            multiple,
                        })}>
                        {(selected as OptionProps<T>[]).length > 0 ? this._renderSelectionMultiple() : placeholder}
                    </Flex>
                ) : (
                    <div
                        className={classNames('select-selection-content', {
