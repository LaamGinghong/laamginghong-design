...
import { NotificationType, NotificationConfigOptions, notificationConfig, iconMap } from '../config'
...
const Notification: FC<NotificationProps> = ({ type, title, description, container, node }) => {
...
export default {
    create(config: NotificationCreateConfig & { type: NotificationType }): void {
        ...
        render(<Notification type={type} description={description} container={container} node={node} title={title} />, node)
        setTimeout(() => {
            unmountComponentAtNode(node)
            container.removeChild(node)
        }, duration)
